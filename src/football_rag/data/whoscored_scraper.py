    ...
    try:
        ...
        try:
            await page.wait_for_selector(
                'script:-soup-contains("matchCentreData")',
                state="attached",
                timeout=30000,
            )
        except Exception:
            print(
                "Warning: Timeout waiting for matchCentreData script, attempting to parse anyway..."
            )
        ...
        df["period_display_name"] = df["period"].apply(
            lambda x: x["displayName"] if isinstance(x, dict) else x
        )
        ...
        df.drop(
            columns=["period", "type", "outcome_type"], inplace=True, errors="ignore"
        )
...
async def collect_all_season_matches(
    page: Page, exclude_match_ids: Optional[set] = None, limit: Optional[int] = None
):
...
async def scrape_complete_season_async(
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            all_match_urls = await collect_all_season_matches(
                page, exclude_match_ids, limit
            )
...
def save_matches_locally(
    matches_df: pd.DataFrame, league: str = "eredivisie", season: str = "2025-2026"
):
    output_dir = Path(f"data/raw/whoscored_matches/{league}/{season}")
    ...
    for match_url in matches_df["match_url"].unique():
        ...
        match_events = matches_df[matches_df["match_url"] == match_url].to_dict(
            orient="records"
        )
        match_data = {
            "match_id": match_id,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--limit", type=int, help="Number of matches to scrape (for n_matches mode)"
    )
    ...
    season_data = loop.run_until_complete(
        scrape_complete_season_async(args.mode, args.limit, args.league, args.season)
    )
