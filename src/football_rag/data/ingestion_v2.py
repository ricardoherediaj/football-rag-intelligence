...
def load_whoscored_team_ids() -> Dict[str, int]:
    ...
    csv_path = (
        ...
        / "data"
        / "raw"
        / "eredivisie_whoscored_team_ids.csv"
    )
...
def load_match_mapping(minio_client: MinIOClient) -> List[UnifiedMatch]:
    ...
    mapping_obj = minio_client.download_file(
        "mappings/eredivisie/2025-2026/match_mapping.json"
    )
    ...
    for unified_id, match_info in mapping_data.items():
        ws_path = (
            f"whoscored/eredivisie/2025-2026/match_{match_info['whoscored_id']}.json"
        )
...
def calculate_stats(
    events: list, shots: list, ws_team_id: int, fm_team_id: int
) -> Dict[str, Any]:
    ...
    xg_values_clean = [
        x for x in xg_values if not (isinstance(x, float) and np.isnan(x))
    ]
...
def ingest_all(test_mode: bool = True, count: int = 5):
    ...
    store = VectorStore(
        host=settings.database.chroma_host, port=settings.database.chroma_port
    )
    ...
    for i, m in enumerate(matches, 1):
        ...
        ingest_match(
            m.whoscored_path, m.fotmob_path, match_info, ws_team_ids, minio, store
        )
