...
class TacticalMetrics(BaseModel):
    ...
    # Shape & territory
    home_compactness: float = 0.0
    away_compactness: float = 0.0
    home_field_tilt: float = 0.0
    away_field_tilt: float = 0.0
    ...
    def to_prompt_variables(self, context: MatchContext) -> Dict[str, Any]:
        ...
        return {
            ...
            # Shape & territory
            "home_compact": round(self.home_compactness, 1),
            "away_compact": round(self.away_compactness, 1),
            "home_tilt": round(self.home_field_tilt, 1),
            "away_tilt": round(self.away_field_tilt, 1),
