...
def load_prompt(version_key: str = "v3.5_balanced") -> Dict[str, str]:
    ...
    if not prompt_path:
        ...
        existing_files = (
            [f.name for f in prompts_dir.glob("*")]
            if prompts_dir.exists()
    ...
    try:
        ...
        if not version_data:
            ...
            raise KeyError(
                f"Version '{version_key}' not found in {prompt_path.name}. Available: {valid_keys}"
            )

        logger.info(
            f"✅ Loaded prompt version: {version_key} (Score: {version_data.get('score')})"
        )
